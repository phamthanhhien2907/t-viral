      ...
      <div className="relative w-full mx-auto ">
        <div className="sticky w-full top-0">
          <div className="w-full h-[50px] bg-profileColor">
            <ChevronLeft
              ...
              className="absolute top-2 z-30 left-4 max-sm:left-1 text-white cursor-pointer"
              size={30}
            />
            <span className=" text-xl text-white absolute top-2 left-[40%] max-sm:left-[30%] max-sm:text-lg">
              Lịch sử đánh giá chung
            </span>
          </div>
        </div>
          <form
        ...
        <div className="h-screen overflow-y-scroll tabs-list overflow-x-hidden">
          {lottery?.length > 0 ? lottery?.map((el, index) => (
            <div
              key={index}
              className="w-full border-b-2 border-b-black bg-white flex flex-col gap-1 px-2 py-2 rounded-xl mx-2"
            >
